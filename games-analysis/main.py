...
df_main = pd.read_cvs("game-ratings-by-release-dates.csv")

df_main["first_release_date"] = pd.to_datetime(df_main["first_release_date"])

plt.scatter(df_main["first_release_date"], df_main["critic_ratings_value"], color = "blue", label = "Critic Ratings")
...
plt.bar(df_follow.index - BAR_WIDTH / 2, df_follow["total_follows"], color = "blue", label = "Number of Followss", width = BAR_WIDTH)
